# Dominik Hülse

This repository contains the source code for my personal academic website.

🌐 **Website:** https://domhu.github.io

## About

I am an Earth System Scientist and marine biogeochemical modeller at MARUM – Center for Marine Environmental Sciences, University of Bremen. My research focuses on the long-term carbon cycle, marine sediments, ocean biogeochemistry, and Earth system modelling.

## Website

The website includes:

- Research overview
- Publications
- Projects
- Curriculum Vitae
- Teaching
- Contact information

## Built with

This website is built using the excellent [al-folio](https://github.com/alshedivat/al-folio) Jekyll theme and is hosted with GitHub Pages.

## License

The website content is © Dominik Hülse.

The website framework is based on the MIT-licensed al-folio template.
